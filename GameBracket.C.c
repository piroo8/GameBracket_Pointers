...
PlayerRecord* create_leaf_record(Player* p){
	...
	*/
    struct PlayerRecord *p_r = malloc(sizeof(struct PlayerRecord));
    p_r -> player = p;

    p_r -> parent = NULL;
    p_r -> left_child = NULL;
    p_r -> right_child = NULL;

    p_r -> game_records[WINS] = 0; // Wins left
    p_r -> game_records[LOSSES] = 0; // Losses Right 
...
PlayerRecord* add_match(
    ...
    Updates the left_child and right_child 
	...
	*/
    struct PlayerRecord *match = malloc(sizeof(struct PlayerRecord));
    ...
    if (p1_wins > p2_wins) {
        match -> left_child = p2;
        match -> right_child = p1;
        match -> player = p1 -> player;//name

        match -> game_records[WINS] = p1_wins;
        match -> game_records[LOSSES] = p2_wins;
    }
    else{
        match -> left_child = p1;
        match -> right_child = p2;

        match -> player = p2 -> player;//name

        match -> game_records[WINS] = p2_wins;
        match -> game_records[LOSSES] = p1_wins;
    }
...
int levels_from_parent (PlayerRecord* root){
    PlayerRecord *curr = root;
    int level = 0;
...
int get_player_rank(char player_id[], PlayerRecord* root){
	...
	*/
    if (strcmp ((root->player->id),player_id) == 0){
        int rank = levels_from_parent(root) + 1;
        return rank;
    }
    ...
    return get_player_rank(player_id, root->left_child) + get_player_rank(player_id, root->right_child);
}
